...
Sub Fastload_Callback()

    Call SortSheetUp(1)
    flidCol = FindColumnHeader("_fl_id")
    LastCol = RowLastColumn(1)
    For i = flidCol To LastCol
        '
        ' in case the header has a reserved word
        '
        If (left(Cells(1, i).Text, 2) = "a_") Then
            Cells(1, i) = Right(Cells(1, i), Len(Cells(1, i)) - 2)
        End If
        Columns(i).NumberFormat = Cells(2, i).Text
    Next i
