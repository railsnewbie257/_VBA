...
Dim t As Integer
    ...
    DBGlbRecordsFound = -1
    DBGlbRecordsToRead = -1
    DBGlbAdodbError = False
    '
    ' Show QUERY form ----------------------------------------------------------------------------
    '
    Call StatusbarDisplay("DBQuery: Start")
    If IsMissing(useQuery) Then
...
100     DBRs.Open GLBUserQuery
        ...
        '
        Call StatusbarDisplay("DBQuery: Prepare Download.")
        DBGlbRecordsFound = -1
    ...
    End If
    '
    If WorksheetFunction.CountA(Cells) <> 0 And Not GLBSameSheet And Not GLBManualPlacement Then
        Sheets.Add
    ...
    If Not GLBDownloadByColumn Then ' Download By Row  ---------------------------------------------------------------------
        ...
        For i = 0 To DBGlbRecordsToRead - 1
            For j = 0 To fieldCount - 1
                Workbooks(WBDownload).Worksheets(SHDownload).Cells(useRow, useCol + j) = Trim(DBRs.Fields(j).Value)
            Next j
            DBRs.MoveNext
            useRow = useRow + 1
